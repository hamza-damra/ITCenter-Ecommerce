...
<head>
    ...
    <title><?php echo e(__('Database Setup')); ?> - Bootstrap Mode</title>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="header-right">
                ...
                <a href="<?php echo e(route('admin.bootstrap.logout')); ?>" class="btn btn-secondary">
        ...
        <div class="card">
            ...
            <div class="status-grid" id="statusGrid">
                <div class="status-item <?php echo e($stateInfo['state'] === 'missing_db' ? 'warning' : 'danger'); ?>">
                    <div class="status-item-label">Database State</div>
                    <div class="status-item-value"><?php echo e($stateInfo['state_label']); ?></div>
                </div>
                <div class="status-item info">
                    <div class="status-item-label">Target Database</div>
                    <div class="status-item-value"><?php echo e($stateInfo['database'] ?? 'Not configured'); ?></div>
                </div>
                <div class="status-item info">
                    <div class="status-item-label">MySQL Host</div>
                    <div class="status-item-value"><?php echo e($stateInfo['host'] ?? 'Unknown'); ?>:<?php echo e($stateInfo['port'] ?? '3306'); ?></div>
                </div>
            </div>
        </div>
                    ...
                    <div class="file-upload" onclick="document.getElementById('sqlFile').click()">
                        ...
                        <div>
                            ...
                            <div style="font-size: 12px; margin-top: 8px; color: var(--secondary);">
                                SQL files only (Max: <?php echo e(config('backup.max_upload_size', 512)); ?>MB)
                            </div>
                        </div>
        ...
        <div class="card">
            ...
            <?php if(count($availableBackups) > 0): ?>
                    ...
                    <?php $__currentLoopData = $availableBackups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $backup): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="backup-item">
                            <div class="backup-item-info">
                                <div class="backup-item-name"><?php echo e($backup['filename']); ?></div>
                                <div class="backup-item-meta">
                                    Size: <?php echo e(number_format($backup['size'] / 1024 / 1024, 2)); ?> MB | 
                                    Created: <?php echo e($backup['created_at'] ?? 'Unknown'); ?>

                                </div>
                            </div>
                            <button onclick="restoreBackup('<?php echo e($backup['filename']); ?>')" class="btn btn-success">
            ...
            <div style="margin-top: 24px; border-top: 2px solid var(--border); padding-top: 24px;">
                ...
                <div class="file-upload" onclick="document.getElementById('backupFile').click()">
                    ...
                    <div>
                        ...
                        <div style="font-size: 12px; margin-top: 8px; color: var(--secondary);">
                            SQL, ZIP, or GZ files (Max: <?php echo e(config('backup.max_upload_size', 512)); ?>MB)
                        </div>
                    </div>
    ...
    <script>
        ...
        document.getElementById('importSqlForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('<?php echo e(route("admin.bootstrap.import-sql")); ?>', {
                    method: 'POST',
                    headers: {
        ...
        async function restoreBackup(filename) {
            ...
            try {
                ...
                const response = await fetch('<?php echo e(route("admin.bootstrap.restore-backup")); ?>', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken
                    },
                ...
                if (data.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = data.redirect || '<?php echo e(route("admin.login")); ?>';
                    }, 2000);
                } else {
        ...
        async function checkStatus() {
            try {
                const response = await fetch('<?php echo e(route("admin.bootstrap.status")); ?>');
                const data = await response.json();
...
<?php /**PATH C:\Users\Hamza Damra\Documents\ITCenter-Ecommerce\resources\views/admin/bootstrap/setup.blade.php ENDPATH**/ ?>