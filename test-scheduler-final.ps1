# ========================================
# AUTOMATED BACKUP SCHEDULER TEST
# Testing 30-second backup intervals
# ========================================
...
$schedulerProcess = Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd 'c:\Users\Hamza Damra\ITCenter-Ecommerce'; Write-Host 'Scheduler Worker Started' -ForegroundColor Green; php artisan schedule:work" -PassThru -WindowStyle Normal
...
Write-Host "Monitoring for 120 seconds (expecting 4 backups)..." -ForegroundColor Cyan
...
Write-Host "  Duration: 120 seconds" -ForegroundColor White
Write-Host "  Schedule: Every 30 seconds" -ForegroundColor White
Write-Host "  Expected: 4 backups" -ForegroundColor White
Write-Host ("  Actual: {0} backups" -f $totalCreated) -ForegroundColor $(if ($totalCreated -ge 3) { "Green" } elseif ($totalCreated -gt 0) { "Yellow" } else { "Red" })
Write-Host ""
...
if ($totalCreated -ge 4) {
    ...
    Write-Host "PARTIAL SUCCESS: Created $totalCreated backups (expected 4)" -ForegroundColor Yellow
    Write-Host "Scheduler is working but may need timing adjustment" -ForegroundColor Yellow
} elseif ($totalCreated -gt 0) {
    Write-Host "LIMITED SUCCESS: Created only $totalCreated backup(s)" -ForegroundColor Yellow
} else {
