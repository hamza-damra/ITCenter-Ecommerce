...
<body data-t-request-product="{{ __t('messages.request_product') }}" data-t-contact-us="{{ __t('messages.contact_us') }}">
    ...
    <button class="mobile-search-btn" id="mobileSearchBtn" aria-label="{{ __t('messages.search') }}" type="button">
        ...
        <div class="header-container">
            ...
            <div class="mobile-header-icons">
                ...
                <a href="{{ route('cart.index') }}" class="mhi-btn mhi-cart-link">
                    <i class="fas fa-shopping-cart"></i>
                    <span class="mhi-badge {{ $mobileCartCount > 0 ? '' : 'hidden' }}" id="mhi-cart-count">{{ $mobileCartCount }}</span>
                </a>
                @guest
                ...
                @else
                <a href="{{ route('profile.index') }}" class="mhi-btn" aria-label="{{ __t('messages.my_profile') }}">
                    <i class="fas fa-user"></i>
                </a>
                @endguest
                <button type="button" class="mhi-btn" id="mobileHeaderSearchBtn" aria-label="{{ __t('messages.search') }}">
                    <i class="fas fa-search"></i>
                </button>
            </div>
            ...
            <div class="header-icons">
                ...
                <div class="header-icon" style="position: relative;">
                    <a href="{{ route('login') }}" style="color: inherit; text-decoration: none; display: flex; align-items: center; gap: 0.5rem;" aria-label="{{ __t('messages.login') }}">
                        <i class="fas fa-user"></i>
                    </a>
                </div>
                @else
                <div class="header-icon user-dropdown" style="position: relative;">
                    ...
                    <div class="user-dropdown-menu" style="display: none; position: absolute; top: calc(100% + 10px); inset-inline-end: 0; background: #ffffff; backdrop-filter: blur(10px); border: 2px solid #e8eef7; border-radius: 12px; min-width: max-content; box-shadow: 0 8px 24px rgba(39, 98, 243, 0.12); overflow: hidden; z-index: 1001; opacity: 0; transform: translateY(-10px); transition: opacity 0.3s ease, transform 0.3s ease;">
                        <a href="{{ route('profile.index') }}" class="user-menu-item" style="width: 100%; display: flex; align-items: center; gap: 0.8rem; padding: 0.9rem 1.2rem; background: none; border: none; cursor: pointer; transition: background 0.3s ease, padding 0.3s ease; text-align: start; font-family: inherit; font-size: inherit; text-decoration: none; color: inherit; white-space: nowrap;">
                            <i class="fas fa-user"></i>
                            <span>{{ __t('messages.my_profile') }}</span>
                        </a>
                        <a href="{{ route('orders.index') }}" class="user-menu-item" style="width: 100%; display: flex; align-items: center; gap: 0.8rem; padding: 0.9rem 1.2rem; background: none; border: none; cursor: pointer; transition: background 0.3s ease, padding 0.3s ease; text-align: start; font-family: inherit; font-size: inherit; text-decoration: none; color: inherit; white-space: nowrap;">
                            <i class="fas fa-box"></i>
                            <span>{{ __t('messages.my_orders') }}</span>
                        </a>
                        <form action="{{ route('logout') }}" method="POST" style="margin: 0;">
                ...
                <div class="header-icon">
                    <a href="{{ route('favorites') }}" style="color: inherit; text-decoration: none;" aria-label="{{ __t('messages.favorites') }}">
                        <i class="fas fa-heart"></i>
                        @php
                ...
                @auth
                <div class="header-icon">
                    <a href="{{ route('orders.index') }}" style="color: inherit; text-decoration: none; position: relative;" title="{{ __t('messages.my_orders') }}" aria-label="{{ __t('messages.my_orders') }}">
                        <i class="fas fa-box"></i>
                    </a>
                </div>
                @endauth
                <div class="header-icon">
                    <a href="{{ route('cart.index') }}" style="color: inherit; text-decoration: none;" aria-label="{{ __t('messages.cart') }}">
                        <i class="fas fa-shopping-cart"></i>
                        @php
    ...
    <div class="social-icons-toggle" onclick="toggleMobileSocial()" role="button" aria-label="{{ __t('messages.share') }}" tabindex="0">
        ...
        <div class="footer-content">
            ...
            <div class="footer-section">
                ...
                <ul>
                    ...
                    <li><i class="fas fa-map-marker-alt"></i><span style="color: #94a3b8;">{{ __('messages.location') }}</span></li>
                </ul>
            </div>
        </div>
