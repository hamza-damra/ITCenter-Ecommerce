...
<div class="admin-hero">
    <div class="admin-hero-content">
        <div class="admin-hero-text">
            ...
            <div>
                <h1>{{ __('messages.edit_role') }}: {{ $role->name }}</h1>
                <p>{{ __('messages.edit_role_subtitle') }}</p>
            </div>
        </div>
        ...
        <div class="card-body">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $role->name) }}" placeholder="{{ __('messages.role_name_placeholder') }}" required>
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox-group">
                        <input type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', $role->is_active) ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3" placeholder="{{ __('messages.role_description_placeholder') }}">{{ old('description', $role->description) }}</textarea>
                @error('description')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
                    ...
                    <div class="permission-group-card {{ $groupHasChecked ? '' : 'collapsed' }}" data-group-card="{{ $groupKey }}">
                        ...
                        <div class="permission-group-body">
                            @foreach($group['permissions'] as $permKey => $permLabel)
                                ...
                                @endphp
                                <div class="permission-checkbox {{ $isChecked ? 'checked' : '' }}">
